/*
  The Array truly is a beatiful thing. For this mini assignment we're going to have you go through some of the commonly used array methods. There are methods built onto the Array prototype so here is a refernce of where you can learn more about them.
...
// console.log(JSON.stringify(justiceLeague));
...
filteredMiddleEarth = middleEarth.filter(i => i!==`Sauraman`);
// console.log(filteredMiddleEarth);
...
const numbers10 = numbers.map(item => item*10);
...
const femaleFriends = friends.filter(item => item.gender===`F`);
const maleFriends = friends.filter(item => item.gender===`M`);
