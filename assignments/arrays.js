...
let carModels = [], webCarModelList = [];
for (let i in inventory) carModels.push(inventory[i].car_model);
carModels = carModels.sort();
for (let i in carModels) {
    if(i===0) { webCarModelList.push(carModels[i]) }
    else { if(carModels[i]!==carModels[i-1]) { webCarModelList.push(carModels[i]) } }
};
console.log(`${webCarModelList.length} Available Car Models:\n${webCarModelList.join(`,\n`)}.`);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [], webCarYearList = [];
for (let i in inventory) carYears.push(inventory[i].car_year);
carYears = carYears.sort();
for (let i in carYears) {
    if(i===0) { webCarYearList.push(carYears[i]) }
    else { if(carYears[i]!==carYears[i-1]) { webCarYearList.push(carYears[i]) } }
}
console.log(`Vehicles available from the following years:\n${webCarYearList.join(`,\n`)}.`);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars = [];
for (let i in carYears) if (carYears[i]<2000) { oldCars.push(carYears[i]) }
console.log(`There are ${oldCars.length} cars in the lot manufactured before 2000.`); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];
for (let i in inventory) { if (inventory[i].car_make===`BMW` || inventory[i].car_make===`Audi`) BMWAndAudi.push(inventory[i]) }
